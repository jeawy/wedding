...
def list_users(request):
    ...
    if isMble:
        return render(request, 'lechi/m_users.html', context)
    else:
        return render(request, 'lechi/users.html', context)
...
def usercenter(request):
    ...
    if  request.user.is_anonymous():
        return comm.redirect_login_path(isMobile, request)
    mykbs = Article.objects.filter(Q(author = request.user) & ~Q(status = 3))
    kb_counter = len(Article.objects.all())
    committee = request.user.has_perm('kb.manage_all_kb')
    content = {
        'mykbs' : mykbs,
    ...
    if isMobile:
        return render(request, 'admin_user/m_usercenter.html', content)
    else:
        return render(request, 'admin_user/usercenter.html', content)